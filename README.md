...
benchmarking raidz1-3 and dRAID1-3 IO and resilvering performance, using HDD, SSD or NVMe, various stripe width, DRAM size, zpool utilization, and so on...
...
- inject error to disk so ZFS will treat it as failure disk
	...
	- or, erase first and last few MB of disk(FASTER).e.g., 
	```
	DEV=sdx ; 
	dd bs=512 if=/dev/zero of=/dev/$DEV count=$((2048*500)) seek=$((`blockdev --getsz /dev/$DEV` - 2048*500 )) & 
	dd bs=512 if=/dev/zero of=/dev/$DEV count=$((2048*500)) & 
	``` 
